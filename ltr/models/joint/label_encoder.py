...
class ResidualDS16SW(nn.Module):
    ...
    def __init__(self, layer_dims, use_bn=True):
        ...
        self.res1_tm = BasicBlock(layer_dims[0], layer_dims[1], stride=2, downsample=ds1_tm, use_bn=use_bn)
        ds2_tm = nn.Conv2d(layer_dims[1], layer_dims[2], kernel_size=3, padding=1, stride=2)
        self.res2_tm = BasicBlock(layer_dims[1], layer_dims[2], stride=2, downsample=ds2_tm, use_bn=use_bn)

        ds1_tr = nn.Conv2d(layer_dims[0], layer_dims[1], kernel_size=3, padding=1, stride=2)
        self.res1_tr = BasicBlock(layer_dims[0], layer_dims[1], stride=2, downsample=ds1_tr, use_bn=use_bn)
        ds2_tr = nn.Conv2d(layer_dims[1], layer_dims[2], kernel_size=3, padding=1, stride=2)
        self.res2_tr = BasicBlock(layer_dims[1], layer_dims[2], stride=2, downsample=ds2_tr, use_bn=use_bn)
        ...
        self.label_pred_tr = conv_block(layer_dims[2], layer_dims[3], kernel_size=3, stride=1, padding=1,
                                        relu=True, batch_norm=use_bn)
    ...
    def forward(self, label_mask, feature=None):
        ...
        out_tr = self.res2_tr(self.res1_tr(out))

        label_enc_tm = self.label_pred_tm(out_tm)
        sample_w_tm = self.samp_w_pred(out_tm)

        label_enc_tr = self.label_pred_tr(out_tr)
        ...
        return label_enc_tm, label_enc_tr, sample_w_tm
