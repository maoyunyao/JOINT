...
import ltr.models.joint.transformer as tfmer
import ltr.models.joint.linear_filter as target_clf
import ltr.models.target_classifier.features as clf_features
import ltr.models.joint.initializer as seg_initializer
import ltr.models.joint.label_encoder as seg_label_encoder
import ltr.models.joint.loss_residual_modules as loss_residual_modules
import ltr.models.joint.decoder as seg_decoder
...
class JOINTNet(nn.Module):
    def __init__(self, feature_extractor, transformer, target_model, combiner, decoder,
                       target_model_input_layer, decoder_input_layers, label_encoder=None):
        super().__init__()

        self.feature_extractor = feature_extractor      # Backbone feature extractor F
        self.transformer = transformer                  # Transformer as transduction module
        self.target_model = target_model                # Target model and the few-shot learner
        self.combiner = combiner                        # Mask Encoding combiner
    ...
    def forward(self, train_imgs, test_imgs, train_masks, test_masks, num_refinement_iter=2):
        ...
        test_feat_backbone = self.extract_backbone_features(
            test_imgs.reshape(-1, test_imgs.shape[-3], test_imgs.shape[-2], test_imgs.shape[-1]))
        ...
        # Extract features input to the transformer
        train_feat_bb = self.get_backbone_target_model_features(train_feat_backbone)  # seq*frames, channels, height, width
        train_feat_bb = train_feat_bb.view(num_train_frames, num_sequences, *train_feat_bb.shape[-3:])

        train_feat_tm_all = [train_feat_tm, ]
        train_feat_bb_all = [train_feat_bb, ]

        # Get few-shot learner label and spatial importance weights
        few_shot_label_tm, few_shot_label_tr, few_shot_sw_tm = self.label_encoder(train_masks, None)

        few_shot_label_tm_all = [few_shot_label_tm, ]
        few_shot_label_tr_all = [few_shot_label_tr, ]
        few_shot_sw_tm_all = None if few_shot_sw_tm is None else [few_shot_sw_tm, ]

        test_feat_tm = self.extract_target_model_features(test_feat_backbone)  # seq*frames, channels, height, width
        test_feat_bb = self.get_backbone_target_model_features(test_feat_backbone)  # seq*frames, channels, height, width

        # Obtain the target module parameters using the few-shot learner
        filter, filter_iter, _ = self.target_model.get_filter(train_feat_tm, few_shot_label_tm, few_shot_sw_tm)
        ...
        for i in range(num_test_frames):
            ...
            test_feat_bb_it = test_feat_bb.view(num_test_frames, num_sequences, *test_feat_bb.shape[-3:])[i:i+1, ...]
            ...
            mask_encoding_pred = [self.target_model.apply_target_model(filter, test_feat_tm_it)]
            ...
            # Apply the transformer to obtain mask encodings.
            if i == 0:
                train_feat_bb_it = train_feat_bb
                few_shot_label_tr_it = few_shot_label_tr
            mask_encoding_transfered = self.transformer(test_feat_bb_it, train_feat_bb_it, train_mask_enc=few_shot_label_tr_it)

            # Combine the mask encoding
            mask_encoding_cmb = mask_encoding_pred_last_iter + mask_encoding_transfered

            # Run decoder to obtain the segmentation mask
            mask_pred, decoder_feat = self.decoder(mask_encoding_cmb, test_feat_backbone_it, test_imgs.shape[-2:])
            ...
            few_shot_label_tm, few_shot_label_tr, few_shot_sw_tm = self.label_encoder(mask_pred_prob, test_feat_tm_it)

            # Extend the training data using the predicted mask
            few_shot_label_tm_all.append(few_shot_label_tm)
            few_shot_label_tr_all.append(few_shot_label_tr)
            if few_shot_sw_tm_all is not None:
                few_shot_sw_tm_all.append(few_shot_sw_tm)

            train_feat_tm_all.append(test_feat_tm_it)
            train_feat_bb_all.append(test_feat_bb_it)
            ...
            if (i < (num_test_frames - 1)) and (num_refinement_iter > 0):
                ...
                train_feat_bb_it = torch.cat(train_feat_bb_all, dim=0)
                few_shot_label_tm_it = torch.cat(few_shot_label_tm_all, dim=0)
                few_shot_label_tr_it = torch.cat(few_shot_label_tr_all, dim=0)

                if few_shot_sw_tm_all is not None:
                    few_shot_sw_tm_it = torch.cat(few_shot_sw_tm_all, dim=0)
                else:
                    few_shot_sw_tm_it = None
                ...
                filter_updated, _, _ = self.target_model.filter_optimizer(TensorList([filter]),
                                                                          ...
                                                                          label=few_shot_label_tm_it,
                                                                          sample_weight=few_shot_sw_tm_it,
        ...
        few_shot_label_tm_all = torch.cat(few_shot_label_tm_all, dim=0)
        few_shot_label_tr_all = torch.cat(few_shot_label_tr_all, dim=0)

        return mask_predictons_all, few_shot_label_tm_all, few_shot_label_tr_all

    def segment_target(self, target_filter, test_feat_tm, test_feat, mask_encoding_transfered):
        ...
        mask_encoding_cmb = mask_encoding_pred + mask_encoding_transfered

        mask_pred, decoder_feat = self.decoder(mask_encoding_cmb, test_feat,
...
def steepest_descent_resnet50(filter_size=1, num_filters=1, optim_iter=3, optim_init_reg=0.01,
    ...
    # Extracts features input to the transformer
    transformer_feature_adjustor = clf_features.residual_basic_block(
        feature_dim=layer_channels[target_model_input_layer],
        num_blocks=clf_feat_blocks, l2norm=False,
        final_conv=final_conv, out_dim=out_feature_dim)
    ...
    residual_module = loss_residual_modules.Residual(init_filter_reg=optim_init_reg,
    ...
    # Transformer
    transformer = tfmer.Transformer(feature_dim=out_feature_dim, key_dim=128,
                                    feature_adjustor=transformer_feature_adjustor,
    ...
    decoder = seg_decoder.SegDecoder(num_filters, decoder_mdim, decoder_input_layers_channels, use_bn=True)

    net = JOINTNet(feature_extractor=backbone_net, transformer=transformer, target_model=target_model,
                  combiner=None, decoder=decoder, label_encoder=label_encoder,
