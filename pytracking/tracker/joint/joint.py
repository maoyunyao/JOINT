...
class JOINT(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if self.frame_num > 2:
            ...
            if self.params.get('update_target_model', True):
                ...
                self.update_target_model(self.prev_test_x_bb, prev_segmentation_prob_crop.clone())
        ...
        test_x_bb = self.net.get_backbone_target_model_features(backbone_feat)
        ...
        segmentation_scores = self.segment_target(test_x_bb, backbone_feat)

        self.prev_test_x_bb = test_x_bb
    ...
    def segment_target(self, sample_bb_feat, sample_x):
        with torch.no_grad():
            mask_encoding_transferred = self.net.transformer(sample_bb_feat.unsqueeze(1), self.train_feat_bb_it, train_mask_enc=self.few_shot_label_tr_it)

            sample_tm_feat = self.net.target_model.extract_target_model_features(sample_bb_feat)
            segmentation_scores, mask_encoding_pred = self.net.segment_target(self.target_filter, sample_tm_feat,
                                                                              sample_x, mask_encoding_transferred)
    ...
    def init_target_model(self, init_backbone_feat, init_masks):
        ...
        with torch.no_grad():
            x_bb = self.net.get_backbone_target_model_features(init_backbone_feat)
            x = self.net.target_model.extract_target_model_features(x_bb)
            few_shot_label_tm, few_shot_label_tr, few_shot_sw = self.net.label_encoder(init_masks, x.unsqueeze(1))
        ...
        with torch.no_grad():
            self.target_filter, _, losses = self.net.target_model.get_filter(x.unsqueeze(1), few_shot_label_tm,
                                                                             few_shot_sw,
                                                                             num_iter=num_iter)

        # For transformer
        self.train_feat_bb_it = x_bb.unsqueeze(1)
        self.few_shot_label_tr_it = few_shot_label_tr
        # self.net.transformer.init(self.train_feat_bb_it, init_masks)

        # Init memory
        if self.params.get('update_target_model', True):
            self.init_memory(TensorList([x_bb]), masks=init_masks.view(-1, 1, *init_masks.shape[-2:]))

    def update_target_model(self, train_x_bb, mask, learning_rate=None):
        ...
        if self.frame_num % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory(TensorList([train_x_bb]), mask, learning_rate)
            memory_updated = True

        # Decide the number of iterations to run
        num_iter = 0
        if (self.frame_num - 1) % self.params.train_skipping == 0 and memory_updated:
            num_iter = self.params.get('net_opt_update_iter', None)

        if num_iter > 0:
            samples_bb = self.training_samples[0][:self.num_stored_samples[0],...]
            masks = self.target_masks[:self.num_stored_samples[0], ...]

            with torch.no_grad():
                samples = self.net.target_model.extract_target_model_features(samples_bb)
                few_shot_label_tm, few_shot_label_tr, few_shot_sw = self.net.label_encoder(masks, samples.unsqueeze(1))
            ...
            with torch.no_grad():
                target_filter, _, losses = self.net.target_model.filter_optimizer(TensorList([self.target_filter]),
                                                                                  ...
                                                                                  label=few_shot_label_tm,
                                                                                  sample_weight=sample_weights)

            # For transformer
            self.train_feat_bb_it = samples_bb.unsqueeze(1)
            self.few_shot_label_tr_it = few_shot_label_tr
            ...
            # ##### Debug
            # import numpy as np
            # count_array = np.zeros(shape=(100,2))
            # for i in range(100):
            #     count_array[i,0] = ((few_shot_label_tm > i) * (few_shot_label_tm < i + 1)).sum()
            #     count_array[i,1] = ((few_shot_label_tr > i) * (few_shot_label_tr < i + 1)).sum()
            
            # np.savetxt('test_tm.txt', count_array[:,0], fmt='%d')
            # np.savetxt('test_tr.txt', count_array[:,1], fmt='%d')
                
            # import pdb; pdb.set_trace()
            # import cv2
            # import numpy as np
            # tm_show = few_shot_label_tm[0,0].cpu().numpy()
            # for i in range(16):
            #     tmp = tm_show[i, ...]
            #     tmp = tmp - tmp.min()
            #     tmp = tmp / (tmp.max() + 1e-5)
            #     tmp = tmp * 255
            #     tmp_clr =  cv2.applyColorMap(tmp.astype(np.uint8), cv2.COLORMAP_JET)
            #     cv2.imwrite('tm_' + str(i) + '.png', tmp_clr)
            # tr_show = few_shot_label_tr[0, 0].cpu().numpy()
            # for i in range(16):
            #     tmp = tr_show[i, ...]
            #     tmp = tmp - tmp.min()
            #     tmp = tmp / (tmp.max() + 1e-5)
            #     tmp = tmp * 255
            #     tmp_clr =  cv2.applyColorMap(tmp.astype(np.uint8), cv2.COLORMAP_JET)
            #     cv2.imwrite('tr_' + str(i) + '.png', tmp_clr)
            # import pdb; pdb.set_trace()
            # import cv2
            # import numpy as np
            # if not hasattr(self, 'tm_mean_list'):
            #     self.tm_mean_list = []
            #     self.tr_mean_list = []
            
            # tm = few_shot_label_tm[-1,-1]
            # tr = few_shot_label_tr[-1,-1]
            # tm_sum = tm.sum(dim=[1,2])
            # tr_sum = tr.sum(dim=[1,2])
            # tm_nonzero = tm > 0
            # tr_nonzero = tr > 0
            # tm_mean = tm_sum / (tm_nonzero.sum(dim=[1,2]) + 1e-5)
            # tr_mean = tr_sum / (tr_nonzero.sum(dim=[1,2]) + 1e-5)
            # self.tm_mean_list.append(tm_mean.cpu().numpy().tolist())
            # self.tr_mean_list.append(tr_mean.cpu().numpy().tolist())
